...
- Manages system-wide settings (barangay name, zones, streets)
- Views system logs
- **Not documented in the capstone paper — treated as an implementation detail**

### 2. Barangay Official (Admin)
- Creates, updates, and manages waste collection schedules (organized by street name)
- Reviews and responds to resident-submitted reports (missed collection, illegal dumping)
- Monitors collection activity status
- Approves or rejects resident account registrations
- Manages resident participation and points records
...
- Receives **SMS notifications** about upcoming collection schedules and updates
...
- Schedules are organized **by street name**
- Barangay Official can **create, update, and delete** schedules
- Each schedule is assigned to specific **waste collection personnel**
- Residents can view schedules relevant to their registered street

---

## 📲 Notifications

- Delivered via **SMS only** (Semaphore PH)
- Triggered by: upcoming collection schedules, report status updates, account approval
...
1. **Schedule Management** — create/update/delete collection schedules by street
2. **Resident Notification** — SMS alerts for upcoming collections and updates
...
5. **Points-Based Incentive** — manually awarded by collection personnel
6. **Account Approval Workflow** — resident self-register, admin approves
